...
<%
    ...
    // ===== Load promotions đang hiệu lực & nhóm theo brandID =====
    Map<Integer, List<Promotion>> promosByBrand = new HashMap<>();
    PromotionDAO promoDao = null;                                  // dùng lại ở dưới
    java.sql.Date today = new java.sql.Date(System.currentTimeMillis());

    try {
        promoDao = new PromotionDAO();
        List<Promotion> promos = promoDao.getAllPromotions();       // lấy toàn bộ -> tự lọc

        for (Promotion pr : promos) {
            // Chỉ giữ promo active & còn hiệu lực ngày
            if (!pr.isIsActive()) continue;
            if (pr.getStartDate() != null && today.before(pr.getStartDate())) continue;
            if (pr.getEndDate()   != null && today.after(pr.getEndDate())) continue;
...
<head>
    ...
    <style>
        ...
        .hero-section { background-color: #6c757d; color: white; padding: 100px 0; text-align: center; }
        .card-body a { text-decoration: none; font-weight: bold; font-size: 18px; }
        ...
        .price-new { font-size: 1.25rem; font-weight: bold; color: #dc3545; }
        .promo-badge { font-size: 0.85rem; }
        .btn-layout { display: flex; gap: 10px; padding: 10px 15px; justify-content: center; }
        .btn-layout button { width: 100%; padding: 10px; font-weight: bold; border: none; border-radius: 8px; }
    ...
    <div class="row">
        <% if (products != null && !products.isEmpty()) {
               for (Product p : products) {
                   ...
                   // Lấy brandID của sản phẩm (đổi getter nếu tên khác)
                   ...
                   if (brandPromos != null && promoDao != null) {
                       for (Promotion pr : brandPromos) {
                           // ✅ Chỉ xét khi đủ điều kiện: Active + đúng ngày + đúng brand + price ≥ Min Price
                           if (!promoDao.isApplicableForOrder(pr, productBrandId, price, today)) continue;
