...
public class PromotionDAO {
    ...
    public void updatePromotion(Promotion p) throws SQLException {
        String sql = "UPDATE Promotion SET code=?, description=?, discountType=?, discountValue=?, startDate=?, endDate=?, minOrderValue=?, maxUsage=?, usedCount=?, isActive=? WHERE promotionID=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            prepareStatement(ps, p);
